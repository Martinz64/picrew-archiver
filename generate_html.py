...
for cat in categories:
    ...
    for file in fItems:
        ...
        if os.path.isdir(dir+'/'+cat+'/'+file):
            ...
            # cdir is the base path
            items.append((item_thumbnail, variants, cdir, cdir2))
...
with open(dir+'/output.html', 'w', encoding="utf-8") as f:
    f.write('''
    <!DOCTYPE html>
<html lang="en">
<head>
...
<ul>
    ...
    for cat in catgs:
        ...
        f.write('<h3 class="title">' +
                ''.join(cat[2].split('-')[1:]) + '</h3>')
        f.write('</div>')
        ...
        for item in cat[1]:
            ...
            # f.write('</tr>')
            f.write('<div class="thumbnail"><a href="' +
                    item[3] + '/index.html" target="_blank"><img src="' + item[0] + '" /></a></div>')
...
for cat in catgs:
    for item in cat[1]:
        with open(item[2] + '/index.html', 'w', encoding="utf-8") as f:
            f.write(html_header)
            f.write('<img class="thumbnail" src="' +
                    item[0].split('/')[-1] + '" />')
            f.write('<div class="main">')

            for variant in item[1]:
                f.write('<img class="content" src="' +
                        variant.split('/')[-1] + '" />')
            f.write('</div>')
            f.write(html_footer)
